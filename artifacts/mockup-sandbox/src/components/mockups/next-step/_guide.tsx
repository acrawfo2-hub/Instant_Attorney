...
// Mirror of the main app's components/NextStepGuide.tsx. The only change is
// next/link → a plain <a> (routing is irrelevant in the isolated preview).
// Markup and class names are identical so the rendered output matches prod.
...
export function NextStepGuidePreview({
  caseFile,
  documents,
  facts,
  preWarmedByType = {},
          ...
          <div className="lf-nextstep-hero">
            ...
            {(guide.cta || guide.secondary) && (
              <div className="lf-nextstep-actions">
                {guide.cta && (
                  <a href={guide.cta.href} className="lf-nextstep-btn">
                    {guide.cta.label}
                  </a>
                )}
                {guide.secondary && (
                  <a href={guide.secondary.href} className="lf-nextstep-link">
