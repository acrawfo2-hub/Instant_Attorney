...
- **AI providers (in progress)** — **Anthropic (Claude)** remains the default; **xAI (Grok)** is being introduced as a user-selectable workhorse option. **ZDR must be confirmed for each provider before that provider processes public client content.** See `docs/ai-provider-roadmap.md`.
...
- **Vendors** — Anthropic (AI inference, default), xAI (AI inference, optional preference), Supabase (cloud hosting/storage), Stripe (payments), Resend (transactional email).
...
4. **ZDR — confirm before launch.** Do not publish ZDR claims until ZDR is actually enabled with each AI provider that may receive client content (Anthropic today; Anthropic **and** xAI before offering Grok in production).
