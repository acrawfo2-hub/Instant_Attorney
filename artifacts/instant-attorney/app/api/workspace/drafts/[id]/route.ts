...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (promotedDocumentId) {
    ...
    if (revisionAction === "create_revision") {
      ...
      if (error || !revision) return NextResponse.json({ error: "Could not create a safe revision." }, { status: 500 });
      promotedDocumentId = revision.id;
      await startDocumentReview(revision.id, linked.case_file_id);
    } else {
      ...
      if (error) return NextResponse.json({ error: "Could not save the linked document revision." }, { status: 500 });
