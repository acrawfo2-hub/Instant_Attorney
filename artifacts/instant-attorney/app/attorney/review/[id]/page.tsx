        ...
        <div className="atty-review-content">
          ...
          {secondDraft?.draft_text && (
            <section className="atty-standalone-doc atty-second-draft-preview">
              ...
              <div ref={workingEditorRef} className="atty-review-draft arr-working-editor" tabIndex={-1}>
                {(() => {
                  const active = improvements.find((item) => item.id === focusedImprovementId);
                  const anchor = active?.anchor;
                  if (!anchor || secondDraft.draft_text.slice(anchor.start, anchor.end) !== anchor.quote) return renderDocumentText(secondDraft.draft_text);
                  return <pre className="arr-working-text">{secondDraft.draft_text.slice(0, anchor.start)}<mark>{anchor.quote}</mark>{secondDraft.draft_text.slice(anchor.end)}</pre>;
                })()}
              </div>
            </section>
          )}
