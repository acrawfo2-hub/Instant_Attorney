...
export default async function AdminPage() {
  ...
  const { data: profile } = await db
    .from("profiles")
    .select("is_attorney, full_name, email")
    .eq("id", user.id)
    .single();
    ...
    <div className="admin-shell">
      ...
      <main className="admin-main">
        <p className="admin-intro">
          Token caps have been removed pipeline-wide — every model now runs to its full
          output ceiling. This page records each call that <strong>would have been truncated</strong> under
          the old limits, so you can tune prompts and limits without ever shipping a cut-off document.
        </p>
        ...
        <section className="admin-section">
          ...
          {featureSummaries.length === 0 ? (
            <div className="admin-empty">No tracked calls yet. Generate a document to populate this.</div>
          ) : (
            <table className="admin-table">
              ...
              <tbody>
                {featureSummaries.map((f) => (
                  <tr key={f.feature} className={f.wouldHaveHit > 0 ? "admin-tr-warn" : ""}>
                    <td className="admin-td-feature">{featureLabel(f.feature)}</td>
                    <td>{f.tracked}</td>
                    <td>{f.wouldHaveHit > 0 ? <strong>{f.wouldHaveHit}</strong> : 0}</td>
                    <td>{f.stillTruncated > 0 ? <strong className="admin-danger-text">{f.stillTruncated}</strong> : 0}</td>
                    <td>{f.maxOutput.toLocaleString()}</td>
                    <td className="admin-td-muted">{f.priorLimit?.toLocaleString() ?? "—"}</td>
                    <td className="admin-td-muted">{f.modelMax?.toLocaleString() ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section className="admin-section">
          <h2 className="admin-section-title">
            ...
            {recent.length > 0 && <span className="admin-count">{recent.length}</span>}
          </h2>
          {recent.length === 0 ? (
            <div className="admin-empty">Nothing has exceeded an old cap yet.</div>
          ) : (
            <table className="admin-table">
              <thead>
                <tr>
              ...
              <tbody>
                {recent.map((e) => {
                  ...
                  return (
                    <tr key={e.id}>
                      <td className="admin-td-muted">{new Date(e.created_at).toLocaleString()}</td>
                      <td className="admin-td-feature">{featureLabel(e.feature)}</td>
                      <td className="admin-td-muted">{e.model ?? "—"}</td>
                      <td>{out.toLocaleString()}</td>
                      <td className="admin-td-muted">{cap.toLocaleString()}</td>
                      <td><strong>+{over.toLocaleString()}</strong></td>
                      <td>
                        {e.metadata?.still_truncated ? (
                          <span className="admin-badge admin-badge-danger">Yes</span>
                        ) : (
