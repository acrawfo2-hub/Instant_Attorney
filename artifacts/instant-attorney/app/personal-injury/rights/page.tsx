...
import {
  ...
  isKnownPiSituation,
  type PiSituation,
} from "@/lib/pi-claim-guide";
import { getPiInstrument } from "@/lib/pi-instruments";
...
function ClaimGuideTool() {
  const params = useSearchParams();
  const caseFileId = params.get("caseFileId");
  const initial = params.get("situation");
  const [situation, setSituation] = useState<PiSituation | null>(
    initial && isKnownPiSituation(initial) ? initial : null
  );

  const guidance = situation ? piClaimGuideFor(situation) : null;
  const wizardHref = (wizardType: string) =>
    caseFileId ? `/wizard/${wizardType}?caseFileId=${caseFileId}` : `/free-chat?area=personal-injury`;
  ...
  return (
    <div style={{ minHeight: "100vh", background: cream, color: textDk, fontFamily: "var(--font-dm-sans), system-ui, sans-serif" }}>
      <header style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "16px 24px", background: navy, color: "var(--brand-cream-heading)" }}>
        ...
        <span style={{ fontSize: 13, letterSpacing: 0.5, color: gold }}>Injury Claim Guide</span>
      </header>
      ...
      <main style={{ maxWidth: 720, margin: "0 auto", padding: "32px 20px 80px" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(200,169,110,0.14)", color: textMd, padding: "6px 12px", borderRadius: 999, fontSize: 12.5, marginBottom: 20 }}>
          <Shield size={14} color={gold} /> Free · expert PI playbook · changes nothing in your file
        </div>

        <h1 style={{ fontFamily: serif, fontSize: 34, lineHeight: 1.15, margin: "0 0 14px" }}>
          Your injury claim playbook
        </h1>
        <p style={{ fontSize: 16, lineHeight: 1.6, color: textMd, margin: "0 0 22px" }}>
          Injured people are often bombarded by insurers before they understand their rights. Pick what&apos;s
          happening and we&apos;ll lay out what an expert Texas PI attorney would tell you on day one.
        </p>
        ...
        {guidance && (
          <section style={{ marginTop: 24, background: "var(--brand-white)", border: `1px solid ${border}`, borderRadius: 12, padding: 22 }}>
            ...
            <h3 style={{ ...sectionLabel, color: "#b4341f" }}>Do not</h3>
            <ul style={listStyle}>
              ...
              <>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                  {guidance.relevant_instruments.map((key) => {
                    const inst = getPiInstrument(key);
                    if (!inst) return null;
                    return (
                      <Link
                        key={key}
...
export default function PersonalInjuryRightsPage() {
  return (
