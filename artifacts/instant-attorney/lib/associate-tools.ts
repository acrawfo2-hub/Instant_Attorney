...
import type { AssociateShortcutId } from "./associate-shortcuts.ts";

export {
  ASSOCIATE_SHORTCUTS,
  shortcutById,
  type AssociateShortcutId,
} from "./associate-shortcuts.ts";
 ...
 * working copy, approve, waive, or send. Shortcut labels live in
 * associate-shortcuts.ts so the review page can render them without pulling
 * this server graph (attorney-review → next/headers) into a client bundle.
 */
