...
// existing wizard types (no new DocType / DB enum). Mirrors lib/debt-instruments.ts.
//
...
export function defamationInstrumentsForPrompt(): string {
  return DEFAMATION_INSTRUMENTS.map(
    (i) =>
      `• ${i.key} (drafts via ${i.wizard_type})${i.high_stakes ? " [HIGH-STAKES — anti-SLAPP risk, recommend consult]" : ""} — ${i.label}: ${i.purpose}`
  ).join("\n");
}
