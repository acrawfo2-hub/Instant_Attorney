...
export interface BillingGate {
  allowed: boolean;
  /** 'topup_pending' | 'topup_failed' — only set when allowed is false. */
  reason?: "topup_pending" | "topup_failed";
...
/** Reset the monthly allowance bucket when the calendar month rolls over. */
function rolledAllowance(ledger: LedgerRow): { used: number; periodStart: string } {
  const monthStart = new Date(
    Date.UTC(new Date().getUTCFullYear(), new Date().getUTCMonth(), 1)
  ).toISOString();
  if (new Date(ledger.allowance_period_start).getTime() < new Date(monthStart).getTime()) {
    return { used: 0, periodStart: monthStart };
  }
  return {
    used: Number(ledger.allowance_used_usd),
    periodStart: ledger.allowance_period_start,
  };
}
...
export async function accrueUsage(userId: string, costUsd: number): Promise<void> {
  ...
  try {
    ...
    const allowance = rolledAllowance(ledger);

    // Draw from the monthly included allowance first; only the overflow meters.
    const allowanceRemaining = Math.max(0, cfg.includedAllowanceUsd - allowance.used);
    ...
    await serviceDb
      ...
      .update({
        meter_usd: newMeter,
        allowance_used_usd: roundUsd(allowance.used + drawnFromAllowance),
...
export async function getBillingGate(userId: string): Promise<BillingGate> {
  ...
  if (ledger.status === "blocked") {
    return { allowed: false, reason: "topup_failed", status: "blocked", meterUsd, thresholdUsd: cfg.thresholdUsd };
  }
...
export async function retryTopUp(userId: string): Promise<BillingGate> {
  const serviceDb = createServiceClient();
  const ledger = await loadLedger(serviceDb, userId);
  if (ledger.status !== "blocked") return getBillingGate(userId);

  await serviceDb
    .from("top_up_ledger")
    .update({ status: "ok", cycle_id: randomUUID(), updated_at: new Date().toISOString() })
    .eq("user_id", userId);
