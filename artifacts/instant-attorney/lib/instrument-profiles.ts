...
export type InstrumentResolution =
  | { status: "resolved"; profile: InstrumentProfile }
 ...
 */
export function confirmInstrumentProfile(
...
export function resolveInstrumentProfile(name: string): InstrumentResolution {
  const normalized = name.trim().toLowerCase().replace(/[–—]/g, "-").replace(/\s+/g, " ");
  const profile = INSTRUMENT_PROFILES.find((candidate) =>
...
export function formatInstrumentAuthorityBlock(resolution: InstrumentResolution): string {
