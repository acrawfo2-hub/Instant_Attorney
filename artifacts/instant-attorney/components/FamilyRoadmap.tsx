...
import type { FamilyRoadmap as FamilyRoadmapData, RoadmapStage } from "@/lib/family-roadmap";

// ─────────────────────────────────────────────────────────────────────────────
// Family Law Roadmap — a calm, staged map of the process for THIS matter, with a
// best-effort "you are here." Presentational only: the staging is computed by the
// pure lib/family-roadmap.ts. It orients the family tools and documents below it
...
export default function FamilyRoadmap({ roadmap }: { roadmap: FamilyRoadmapData }) {
  ...
  return (
    <div className="lf-card lf-card-full" style={{ borderLeft: `3px solid ${gold}` }}>
      ...
      {roadmap.safety && roadmap.safetyNote && (
        <div
          role="note"
          style={{
            display: "flex",
        ...
        >
          ...
          <span style={{ fontSize: 13.5, lineHeight: 1.55, color: "#7a2417" }}>{roadmap.safetyNote}</span>
        </div>
      )}
