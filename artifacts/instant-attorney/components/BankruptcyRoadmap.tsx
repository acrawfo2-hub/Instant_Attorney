...
import type { BankruptcyRoadmap as BankruptcyRoadmapData, BankruptcyStage } from "@/lib/bankruptcy-roadmap";

// ─────────────────────────────────────────────────────────────────────────────
// Bankruptcy Roadmap — a calm, staged map of the Chapter 7 / 13 process with a
// best-effort "you are here." Presentational only; the staging is computed by the
// pure lib/bankruptcy-roadmap.ts. Mirrors FamilyRoadmap.
// ─────────────────────────────────────────────────────────────────────────────

function StageDot({ status, number }: { status: BankruptcyStage["status"]; number: number }) {
  return (
...
export default function BankruptcyRoadmap({ roadmap }: { roadmap: BankruptcyRoadmapData }) {
  ...
  return (
    <div className="lf-card lf-card-full" style={{ borderLeft: `3px solid ${gold}` }}>
      <div className="lf-card-label">The Bankruptcy Path</div>
      <p className="lf-wizard-hint" style={{ marginBottom: 16 }}>
        Here&apos;s the whole process and where you are. It&apos;s more manageable than most people fear — use it as a
        map, not a deadline.
      </p>
