...
**How to apply:** Any sync `messages.create()` with a high `max_tokens` (here:
attachment-processor.ts and several attorney-only routes at 32k–64k) carries this
same latent failure. Batch-API calls (`messages.batches.create`) are exempt — they
are async by design (used by pre-warm.ts and document-utils auto-review).
